...
if page.startswith("🔬 1"):
    ...
    pick_stay = st.selectbox("Filter by stay", ["(all)"] + [str(int(s)) for s in uniq_stays[:2000]], index=0)
    ...
    def _patch_accelerate_to_avoid_urllib3():
        ...
        def _ensure_minimal_stubs(acc_mod):
            ...
            if not hasattr(utils, "get_balanced_memory"):
                utils.get_balanced_memory = acc_mod.get_balanced_memory
            if not hasattr(utils, "infer_auto_device_map"):
                utils.infer_auto_device_map = acc_mod.infer_auto_device_map
            shim_defaults = {
            ...
            for name, fn in shim_defaults.items():
    ...
    # ----- Sentiment paths (robust & OS-agnostic) -----
    ...
    # Your known absolute sklearn path (with space in folder name)
    ...
    def load_rag():
        ...
        if not index_path.exists() or not meta_path.exists():
            st.warning(f"RAG index not found in '{RAG_DIR}'. Build it:\n  python training/build_rag_index.py")
    ...
    _MBART50_CODES = {"en":"en_XX","hi":"hi_IN","ta":"ta_IN","te":"te_IN","mr":"mr_IN","bn":"bn_IN","kn":"kn_IN","ml":"ml_IN","pa":"pa_IN","ur":"ur_PK"}
    ...
    def get_translator(src, tgt):
        try:
            ...
            mdl.eval()
            return ("marian", tok, mdl, None)
        except OSError:
            pass
        except Exception as e:
            st.warning(f"Marian load failed for {src}->{tgt} ({e}); using MBART-50 fallback.")
        try:
            try:
                ...
                _patch_accelerate_to_avoid_urllib3()
                from transformers import MBart50TokenizerFast, MBartForConditionalGeneration
            src_code = _MBART50_CODES.get(src)
            tgt_code = _MBART50_CODES.get(tgt)
            if not src_code or not tgt_code:
                raise ValueError(f"MBART-50 doesn't support the language pair {src}->{tgt}")
            mbart_name = "facebook/mbart-large-50-many-to-many-mmt"
            tok = MBart50TokenizerFast.from_pretrained(mbart_name)
            mdl = MBartForConditionalGeneration.from_pretrained(mbart_name)
            mdl.eval()
            return ("mbart", tok, mdl, {"src_code": src_code, "tgt_code": tgt_code})
        except Exception as e:
            st.error(f"Failed to load MBART-50 fallback for {src}->{tgt}: {e}")
            raise

    def translate_text(text: str, tok, mdl, backend_info: Dict[str, Any] | None = None) -> str:
        import torch
        if backend_info and "tgt_code" in backend_info:
            src_code = backend_info["src_code"]
            tgt_code = backend_info["tgt_code"]
            tok.src_lang = src_code
            inputs = tok([text], return_tensors="pt", truncation=True, max_length=512)
            forced_bos_id = tok.convert_tokens_to_ids(tgt_code)
            with torch.no_grad():
                gen = mdl.generate(**inputs, max_new_tokens=128, num_beams=4, forced_bos_token_id=forced_bos_id)
            out = tok.batch_decode(gen, skip_special_tokens=True)
            return out[0] if out else ""
        inputs = tok([text], return_tensors="pt", truncation=True, max_length=512)
        with torch.no_grad():
            gen = mdl.generate(**inputs, max_new_tokens=128, num_beams=4)
        out = tok.batch_decode(gen, skip_special_tokens=True)
        return out[0] if out else ""

    # ---------------- Sentiment (HF or sklearn) ----------------
    ...
    def load_sentiment() -> SentimentBackend:
        ...
        # 2) sklearn with robust path resolution (fixes duplicated 'Models\\sentiment' & supports your absolute path)
        try:
            import joblib
            candidates: List[Path] = []

            # 2a) If adapter is present, try its path (fix common typo/duplication)
            if SENT_ADAPTER.exists():
                try:
                    ...
                    if meta.get("type") == "sklearn":
                        ...
                        # fix accidental ".../Models/sentiment/..." → ".../Models/Sentiment_Model/..."
                        fixed = fixed.replace("/Models/sentiment/", "/Models/Sentiment_Model/")
                        p = Path(fixed)
            ...
            # 2b) Default relative to SENT_BASE
            candidates.append((SENT_BASE / "sklearn_model.joblib").resolve())

            # 2c) Your explicit absolute path (with space in folder name)
        ...
        try:
            ...
            class VaderAdapter:
                def __call__(self, text):
                    ...
                    return [{"label": lab, "score": abs(float(s["compound"]))}]
            class SentWrap(SentimentBackend):
                def __init__(self): super().__init__()
                def predict_one(self, text: str) -> Dict[str, Any]:
                    res = VaderAdapter()(text)[0]
    ...
    with tab_chat:
        ...
        if st.button("Answer") and q and index is not None and emb is not None:
            ...
            ans = synthesize_answer(q, hits)  # no cites returned
    ...
    with tab_trans:
        ...
        if st.button("Translate"):
            ...
            out = translate_text(text, tok, mdl, backend_info=info)
            st.success(out)
